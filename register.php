		...
		if (!preg_match("/^[a-zA-Z ]*$/", $name)){
			...
			if (empty($_POST["uname"])) {
				...
				if (empty($_POST["upassword"])) {
					...
					if (empty($_POST["ic"])){
						$icErr = "Please enter your ISBN number";
					}else{
						if(!preg_match("/^[0-9 -]*$/", $ic)){
							$icErr = "Please enter a valid ISBN number";
							$ic = "";
						}else{
							$ic = $_POST['ic'];
...
<form method="post"  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	...
	Adhar Number:<br><input type="text" name="ic" placeholder="xxxx-xxxx-xxxx">
	...
	Mobile Number:<br><input type="text" name="contact" placeholder="">
